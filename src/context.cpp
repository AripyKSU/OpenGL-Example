...
bool Context::Init() {
    ...
    //plane의 vbo를 기준으로 0,1,2 attribute setting
    ...
    //계산된 grassPos로 3번 offset attribute setting
    ...
    //3번 attribute는 instanceId가 바뀔 때마다 달라짐
...
void Context::Render() {
    if (ImGui::Begin("ui window")) {
        ...
        if (ImGui::CollapsingHeader("light", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::DragFloat("ssao raidus", &m_ssaoRadius, 0.01f, 0.0f, 5.0f);
        }
    ...
    //cameraFront, view, projection 계산
    m_cameraFront =
    ...
    for (size_t i = 0; i < m_deferLights.size(); i++) {
        //string을 만들어내는 fmt::format 함수 lights[0].position ...
    ...
    //defer이후 forward 과정 전에 depth buffer를 복사
...
void Context::Reshape(int width, int height) {
    ...
    //defer shading g-buffer, position, normal, albedoSpec
